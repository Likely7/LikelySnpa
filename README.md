![LikelySnap promo](./promo/github-15s/poster.png)

![LikelySnap promo animation](./promo/github-15s/LikelySnap-github-promo.gif)

[English](./README.en.md) | 简体中文

# LikelySnap

不是换皮，是底层重做。LikelySnap 基于 OpenScreen 1.5.0 改造而来，已经重做了录制保存、项目包、摄像头链路和长视频导出，目标很简单：让长录制更安心，录完之后文件真的在，后面还能继续编辑和导出。

## 这个项目是怎么来的

事情一开始非常朴素。

我下载了原版 OpenScreen，心想：这个东西看着不错啊，界面干净，能录屏，能剪，还能做 zoom。于是我兴致勃勃地开录，认认真真录了差不多 40 分钟。

录完以后，我点击停止。

然后，文件没了。

不是说文件损坏了，不是说打不开，也不是说在哪里藏了一个临时文件等我去救。是真的找不到。桌面没有，下载没有，默认目录没有，翻来翻去没有。我甚至开始到处找残留文件，想着哪怕给我留个半截也行。

结果连残留都没有。

那一刻就很离谱。40 分钟的录制，不是 40 秒。人坐在那里讲了半天，鼠标点了半天，东西演示了半天，最后软件跟我说：没有。像什么都没发生过。

我当时真的火大。

所以这个项目不是因为“想做一个新皮肤”，也不是因为“想改个名字发新版”。LikelySnap 是被那次 40 分钟录制消失逼出来的。

我不想要一个短视频玩具。我想要的是：录得久一点也别炸，录完之后文件真的在，打开项目还能剪，摄像头、声音、鼠标都能对上，最后导出时也别把电脑吃死。

于是，我直接把原版从底层开始重构了一遍。

## 最重要的升级：录制和导出先重做，编辑器继续推进

LikelySnap 这次最大的变化，不是换了名字，不是改了颜色，也不是往界面上多塞几个按钮。

真正的变化是：录制保存和导出链路已经换成更适合长视频的处理方式；编辑器目前还没有完全切到 Premiere Pro、DaVinci Resolve、Final Cut Pro 那种 NLE 架构，它仍然是在原版编辑器架构上做可靠性和体验优化。

原版的思路更像“小工具”：录一段，停下来，软件再努力把东西收拾好。短视频还行，一旦录制变长，就容易出事。几十分钟、一小时、甚至更久的视频，不应该靠“内存硬扛”和“最后一刻保存”来赌命。

LikelySnap 现在做的是另一套思路：把一次录制当成一个真正的项目来处理。

Premiere Pro、DaVinci Resolve、剪映这类软件为什么能剪很长的视频、多条轨道、音频波形、预览缓存、导出队列？核心不是它们有魔法，而是它们不会傻乎乎地把所有素材一次性塞进内存里。它们会把素材当成“项目”来管理：文件在磁盘上，时间线按需读取，波形和预览可以缓存，重活放到后台，导出时一边处理一边写文件。

LikelySnap 现在正在往这个方向重做，但这里要说清楚：当前版本还不是完整 NLE。录制和导出已经先走向更稳的项目化、文件化路线；编辑器还保留了原版的浏览器时间线架构，所以长视频打开时仍然可能卡住一段时间。

不是“录完生成一个视频就结束”，而是把屏幕、摄像头、声音、鼠标轨迹、项目信息放进同一个 `.likelysnap` 项目包里。这个项目包可以移动，可以重新打开，可以继续编辑，也更容易做恢复。

打开编辑器这块，目前只是做过一些减负和缓存优化，并没有根治架构问题。30 分钟到 1 小时，甚至 1 小时以上的视频，第一次打开仍然可能明显卡顿，Windows 上尤其可能等得更久。这不是我想吹成“已经秒开”的东西，后续真正要解决，方向应该是把编辑器继续推进到更完整的 NLE 架构。

导出也一样。长视频导出不应该是“先憋一个巨大文件在内存里，最后再保存”。LikelySnap 改成边处理边写文件，成功后再交给你最终 MP4。

这就是为什么我说它不是换皮。

它是把原版最容易在长录制场景里翻车的底层流程拆开，先把录制保存、项目包、摄像头和导出这些关键链路重做，再逐步把编辑体验补上。录制不再赌最后一秒，导出不再把整条片子憋在内存里；编辑器目前仍是阶段性优化，不是最终形态。

### 录制换了

录制过程中就持续写文件，不再把希望全部押在最后点击停止那一秒。

你录屏幕、麦克风、系统声音、摄像头、鼠标轨迹，这些东西都会尽量作为一个项目持续保存，而不是录完后才临时拼命整理。

这意味着长录制的安全感完全不一样：不是录了一个小时以后才开始祈祷软件别炸，而是从录制开始就不断留下可检查、可移动、可继续处理的文件。

### 编辑器还没彻底换

编辑器现在还没有完全换成成熟剪辑软件那种 NLE 架构。

当前做过的事情，是尽量减少一些不必要的加载、缓存部分重数据、让波形和自动 zoom 这些东西不要全部挤在同一个时间点处理。但底层仍然依赖浏览器里的视频元素和前端时间线状态，长视频项目打开时还是可能卡很久。

所以这里不能夸成“打开就能立即操作”。更准确的说法是：LikelySnap 现在能打开和编辑长录制，但 30 分钟到 1 小时以上的项目仍然有明显等待。后续如果要真正接近成熟剪辑软件，编辑器需要继续往 NLE 架构走，让时间线、素材索引、代理、缓存和后台任务成为一套完整系统。

### 导出换了

导出不再走那种容易吃爆内存的老路。

LikelySnap 会把画面处理后持续写入临时文件，确认成功后再变成最终文件。这个思路更接近正经剪辑软件处理长视频的方式：不要把整个结果憋在内存里赌运气，而是让文件一点点安全落地。

对用户来说，区别很直接：长视频导出应该像一个稳定的生产流程，而不是一次巨大的赌博。

## LikelySnap 到底改了什么

一句话：原版更像适合短录制的小工具，LikelySnap 往长录制、可恢复、可继续编辑的方向重做了。

### 1. 录制不再只赌最后一刻

以前最让人崩溃的是：录了很久，最后点停止，才发现前面全白忙。

LikelySnap 改成录制过程中就持续把关键文件写出来。也就是说，它不是等你录完才突然开始整理一大坨东西，而是边录边落盘，尽量让你的录制过程留下真实文件。

这不代表你可以随便断电乱杀进程，但它至少不再是那种“最后一步失败，前面全没了”的恐怖体验。

### 2. 每次录制都是一个项目包

LikelySnap 会把一次录制保存成一个 `.likelysnap` 文件夹。你可以把它理解成“这次录制的项目包”。

里面通常会有：

```text
recording-xxxx.likelysnap/
  screen.mp4
  webcam.mp4
  cursor.json
  manifest.json
```

普通用户不用理解这些文件名。你只要知道：屏幕视频、摄像头、鼠标轨迹、项目信息都放在同一个包里。

所以它更容易移动，也更容易恢复。你把整个 `.likelysnap` 包挪到别的地方，它还是应该能打开。

### 3. 长视频打开仍然是当前边界

原版在长录制面前很容易露怯：东西越大，打开编辑器越痛苦。

LikelySnap 对打开项目做过一些优化，但编辑器还不是完整 NLE 架构，所以不能保证长视频一打开就立刻顺滑操作。

第一次打开很长的视频，仍然可能要等。比如 30 分钟到 1 小时，或者更长的录制，软件需要读取视频信息、准备时间线状态、加载鼠标数据、波形和自动 zoom 相关数据。

目前目标是尽量保证项目能打开、能继续编辑，而不是承诺“秒开”。真正要解决长项目打开卡顿，后续需要继续推进 NLE 化：素材索引、代理文件、后台任务、分层缓存、按需加载，这些才是长期方向。

### 4. 摄像头、声音、鼠标要跟得上

录屏工具最烦人的情况之一是：画面是画面，摄像头是摄像头，声音是声音，鼠标又是另一套东西。短视频还好，一长就容易出错。

LikelySnap 把这些东西当成同一个项目来处理。

它会保存鼠标轨迹，支持后面继续编辑鼠标效果。它也会尽量保证摄像头和主屏幕视频在同一条时间线上。你后面做 zoom、跟随鼠标、导出视频时，这些东西不应该各跑各的。

### 5. 自动 zoom 不再只是傻放大

LikelySnap 保留并强化了自动 zoom。

你可以让它根据鼠标行为生成 zoom 片段，也可以单独调整每个 zoom 片段。需要跟随鼠标的地方，可以跟；不需要跟随的地方，就保持稳定，避免画面一直晃。

简单说：自动生成只是起点，不是判死刑。你后面还能改。

### 6. 导出不再把整段视频憋在内存里

长视频导出最怕什么？怕软件一边导出一边把内存吃满，最后电脑风扇狂转，软件假死，导出失败。

LikelySnap 的 MP4 导出已经改成更适合长视频的方式：边处理，边写文件。它会先写到临时文件，成功后再变成最终文件。

这比“全部憋在内存里，最后一次性保存”靠谱得多。

## 自动 Zoom 的判定逻辑

自动 Zoom 最难的地方不是“能不能放大”，而是“什么时候应该放大”。

录教程和演示时，很多鼠标动作其实不值得放大。比如关闭窗口、点一下按钮、路过某个地方，这些动作如果都生成 Zoom，画面就会一直跳，观感很烦。真正值得放大的，通常是用户正在讲解某个位置，或者正在用鼠标做一个有意图的操作。

LikelySnap 现在的自动 Zoom 判定，不再只看一个简单的鼠标停留点，而是看一段鼠标行为有没有“讲解意图”。

它主要会识别这些情况：

- 鼠标在一个小区域内停留一段时间：适合讲文章、讲界面、讲某个按钮或输入框。
- 鼠标在一个小区域内轻微移动但没有离开：正常人讲解时手会抖、会小范围挪动，这仍然算同一个讲解区域。
- 长时间停留在同一块区域：会生成更长的 Zoom，而不是固定生成一小段后马上跳出去。
- 按住鼠标或拖动：适合白板圈画、框选、划线、拖拽演示，这类 Zoom 默认更适合智能跟随鼠标。
- 重复点击或双击：比孤立单击更像一次有目的的演示动作，所以可以生成短 Zoom。
- 两段自动 Zoom 间隔很近时：会合并成一段，避免刚缩回去又马上放大，画面来回抽动。

它也会主动避开这些情况：

- 单独点一下就离开：大概率只是普通 UI 操作，比如关闭弹窗、切换窗口、点按钮。
- 点击后鼠标马上离开很远：更像路过或完成操作，不应该强行放大。
- 已经有手动 Zoom 的位置：不会再叠一段自动 Zoom 上去抢位置。

### 和原版 OpenScreen 的区别

原版 OpenScreen 的自动 Zoom 更偏“小工具逻辑”：它公开描述的是自动或手动 Zoom，自动 Zoom 会跟随用户工作时的鼠标位置。这个方向很轻，很直接，也适合短演示。

但在长录制里，这种逻辑容易出现两个问题：

- 鼠标在哪里，Zoom 就容易追到哪里，画面会比较忙。
- 如果只看停留或鼠标位置，很难区分“我正在讲这里”和“我只是点了一下这里”。

LikelySnap 的目标不是让画面一直追鼠标，而是让 Zoom 更像剪辑师做出来的镜头选择。

所以 LikelySnap 做了几层改造：

- 自动 Zoom 先判断“这是不是一个值得放大的动作”，再生成片段。
- 孤立单击默认不再触发 Zoom，避免把关闭窗口、点按钮这类动作全放大。
- 长时间讲解同一块区域时，会保留更长 Zoom，不再机械生成固定长度片段。
- 很近的 Zoom 会自动合并，让画面保持一镜到底，而不是缩放来回跳。
- 每个 Zoom 片段都可以单独设置跟随方式：不跟随、智能跟随鼠标、始终跟随鼠标。
- 智能跟随会尽量保持画面稳定，只有鼠标接近放大画面的安全边界时才移动画面。

简单说，原版更像“鼠标在哪就放大哪”，LikelySnap 更像“先判断用户是不是在讲这里，再决定怎么放大、放多久、要不要跟随”。

## 现在能做什么

LikelySnap 现在主要面向这些场景：

- 录制教程、演示、课程、产品说明
- 一次录制几十分钟甚至更久
- 同时录屏幕、麦克风、系统声音、摄像头
- 后期继续调整鼠标效果和 zoom
- 剪掉废话片段，保留重点内容
- 导出 MP4 或 GIF

当前支持的能力包括：

- 屏幕录制和窗口录制
- 麦克风录制
- 系统声音录制
- 摄像头录制
- 鼠标轨迹保存和后期编辑
- 自动 zoom 建议
- 单个 zoom 片段开启或关闭鼠标跟随
- 长视频波形缓存
- 视频剪辑、裁剪、变速、背景、注释、模糊、字幕
- MP4 和 GIF 导出
- macOS 和 Windows x64

## 和原版 OpenScreen 的区别

LikelySnap 不是从零开始的新项目。它基于 OpenScreen 1.5.0 改造而来。

OpenScreen 的优点是：想法好，界面轻，录屏后能快速做一点编辑，对短录制很友好。

但我遇到的问题是：一旦录制时间变长，它就不够稳。长录制需要的不是“看起来能录”，而是每一步都要尽量可恢复、可继续、可验证。

LikelySnap 主要改在这些地方：

- 从“短录制优先”改成“长录制优先”
- 从“停止后再处理一大坨”改成“录制中持续写文件”
- 从“普通视频文件”改成“完整项目包”
- 编辑器仍基于原版架构做阶段性优化，长视频打开卡顿仍是已知问题，后续目标是 NLE 化
- 从“自动 zoom 生成了就算了”改成“每个 zoom 片段还能单独调”
- 从“导出容易吃内存”改成“边处理边写文件”
- 增加独立设置界面，录制位置、项目位置、缓存位置、画质、帧率、码率都能设置

## 长录制说明

LikelySnap 的方向是让长录制更可靠，但长录制本身就是重活。

如果你录了 30 分钟、1 小时、甚至更久，第一次打开项目时，软件可能需要一点时间准备数据。比如生成波形、读取视频信息、准备鼠标预览、生成 zoom 建议。

所以你可能还是会看到几秒到几十秒的等待。区别在于：这个等待应该是“软件在准备”，而不是“软件已经炸了”。

## 当前边界

这个项目还在继续打磨。现在需要诚实说清楚：

- 很长的视频第一次打开仍然可能明显等待，30 分钟到 1 小时以上的项目尤其明显；这属于当前编辑器旧架构的边界。
- GIF 导出不适合超长视频，长视频请优先导出 MP4。
- Windows 端能跑，但不同电脑的显卡、驱动、系统环境差异很大，还需要更多真机测试。
- 多小时级别项目已经明显比原版稳，但仍然需要继续做更多极限测试。

## 开发

安装依赖：

```bash
npm install
```

启动开发：

```bash
npm run dev
```

类型检查：

```bash
npx tsc --noEmit
```

## License

MIT License. See [LICENSE](./LICENSE).
