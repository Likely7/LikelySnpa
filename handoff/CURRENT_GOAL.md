...
- Validate the macOS native webcam finalize fix: with webcam enabled, stop should wait for the webcam writer before the native `recording-stopped` result, `manifest.json` should include `media.webcamVideoPath` only for a valid non-zero `webcam.mp4`, and failed `.sb-*` side-band files should remain in the package for diagnostics/recovery.
