...
11a. macOS native webcam sidecars must not be treated as present until the helper has finalized the webcam writer and verified a non-empty, readable video track. Failed `webcam.mp4` / `.sb-*` artifacts must remain inspectable for recovery instead of being deleted.
...
- Mac webcam short-recording validation is now green after the finalize + PixelBufferAdaptor fixes: `/Users/macbook/Movies/LikelySnap/recording-2026-06-19-20-24-41-248.likelysnap` includes a valid `webcam.mp4` in `manifest.json` with no `.sb-*` files. Next validation should focus on longer macOS webcam recordings and interrupted-session recovery, not another short-path webcam check.
