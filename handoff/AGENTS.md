...
- Current primary platform is macOS. Prioritize ScreenCaptureKit, AVAssetWriter, macOS permissions, and Finder-visible recording directories.
...
- Preserve editor behavior while changing storage: cursor telemetry, auto zoom, manual zoom with auto focus, webcam sidecar video, captions, trim/speed regions, project save/load, and MP4 export must continue to work.
...
- Keep files recoverable. During recording/export, use temp files plus manifest state; on crash, preserve recoverable media rather than deleting partial output.
