...
- macOS native webcam sidecar now records in the ScreenCaptureKit helper with `AVCaptureSession + AVAssetWriter` and writes package-local `webcam.mp4`. Webcam frames are appended through `AVAssetWriterInputPixelBufferAdaptor` from camera pixel buffers rather than by retiming and appending copied camera sample buffers; this fixed the observed mid-writer failure that produced invalid MP4 files and `.sb-*` side-band artifacts.
...
- The recent macOS `webcam.mp4` zero-byte / `.sb-*` failure had two parts: the native stop event could race ahead of webcam finalization, and the webcam writer could still fail mid-recording when appending retimed camera sample buffers. Both are now addressed in code. User retest on `recording-2026-06-19-20-24-41-248.likelysnap` produced a valid `webcam.mp4` with no `.sb-*` files and a manifest `media.webcamVideoPath`.
...
- `swiftc -O -parse-as-library -framework AVFoundation -framework CoreGraphics -framework CoreMedia -framework Foundation -framework ScreenCaptureKit electron/native/screencapturekit/Sources/OpenScreenScreenCaptureKitHelper/main.swift -o electron/native/screencapturekit/build/openscreen-screencapturekit-helper` passes and refreshes the local dev helper after switching macOS native webcam writes to `AVAssetWriterInputPixelBufferAdaptor`.
- Manual macOS webcam retest after the PixelBufferAdaptor fix: latest package `/Users/macbook/Movies/LikelySnap/recording-2026-06-19-20-24-41-248.likelysnap` contains `webcam.mp4` at `49,666,110` bytes, no `.sb-*` side-band files, and `manifest.json` includes `media.webcamVideoPath: "webcam.mp4"`. `ffprobe` reads the webcam sidecar as H.264 1280x720, about `196.99s`, `5893` frames, about `2.0 Mbps`.
